...
### Binary Files
Detected via diff output. Listed in `files` metadata with their status, but binary content is excluded from `raw` diff. The review prompt receives a note like "Binary file changed: image.png".
...
`ignorePaths` from [config](./06-configuration.md) is applied before returning results. Matched files are excluded from both `raw` diff and `files` metadata.
